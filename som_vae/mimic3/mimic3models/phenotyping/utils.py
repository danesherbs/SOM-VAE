...
from mimic3.mimic3models import common_utils
import threading
import random
import os
...
class BatchGen(object):
    ...
    def _load_data(self, reader, discretizer, normalizer, small_part=False, max_seq_len=2000, mask_value=0.):
...
class BatchGenTime(object):

    def __init__(self, reader, discretizer, normalizer, batch_size, max_seq_len, mask_value,
                 small_part, target_repl, shuffle, use_time, return_names=False):
        self.batch_size = batch_size
        self.target_repl = target_repl
        ...
        self._load_data_time(reader, discretizer, normalizer, max_seq_len, mask_value, small_part)

        self.steps = (len(self.data[0]) + batch_size - 1) // batch_size
        self.lock = threading.Lock()
        self.generator = self._generator()
        
    def _load_data_time(self, reader, discretizer, normalizer, max_seq_len=2000, mask_value=0., small_part=False):
        N = reader.get_number_of_examples()
        if small_part:
            N = 1000
        ret = common_utils.read_chunk(reader, N)
    ...
    def _generator(self):
        ...
        while True:
            if self.shuffle:
                ...
                T = self.time
                y = self.data[1]
                (X, T, y, self.names, self.ts) = common_utils.sort_and_shuffle([X, T, y, self.names, self.ts], B)
                self.data = [X, y]
                self.time = T
            ...
            for i in range(0, len(self.data[0]), B):
                ...
                x = common_utils.pad_zeros(x)
                t = common_utils.pad_zeros(t)
                y = np.array(y)  # (B, 25)

                if self.use_time:
                    if self.target_repl:
                        y_rep = np.expand_dims(y, axis=1).repeat(x.shape[1], axis=1)  # (B, T, 25)
                        batch_data = ([x, t], [y, y_rep])
                    else:
                        batch_data = ([x, t], y)
                else:
                    if self.target_repl:
