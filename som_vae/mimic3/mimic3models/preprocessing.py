...
class OneHotEncoder:
    def __init__(self, config_path=os.path.join(os.path.dirname(__file__), 'resources/discretizer_config.json'),
                 start_time='zero', impute_strategy='previous', store_masks=True, max_seq_len=50000, seed=0):
    ...
    def transform(self, X, header=None, end=None):
        ...
        if len(ts) > self._max_seq_len:
            N_bins = self._max_seq_len
            self._timestep = max_hours / N_bins
        else:
            N_bins = len(ts)
        
        data = np.zeros(shape=(N_bins, cur_len), dtype=float)
        mask = np.zeros(shape=(N_bins, N_channels), dtype=int)
        original_value = [["" for j in range(N_channels)] for i in range(len(ts))]
        time = np.zeros(shape=(N_bins,), dtype=float)
        ...
        if len(ts) < self._max_seq_len:
            for i, row in enumerate(X):
                time[i] = float(row[0]) - first_time
                if time[i] > max_hours + eps:
                    continue
                ...
                for j in range(1, len(row)):
                    ...
                    channel_id = self._channel_to_id[channel]
                    mask[i][channel_id] = 1
                    write(data, i, channel, row[j], begin_pos)
                    original_value[i][channel_id] = row[j]
                    
        else:
            idxs = sorted(np.random.choice(range(len(X)), size=self._max_seq_len, replace=False))
        ...
        if self._impute_strategy not in ['zero', 'previous', 'normal_value']:
            raise ValueError("impute strategy is invalid")
        
        if self._impute_strategy in ['previous', 'normal_value']:
            prev_values = [[] for i in range(len(self._id_to_channel))]
            for bin_id in range(N_bins):
                for channel in self._id_to_channel:
                    channel_id = self._channel_to_id[channel]
