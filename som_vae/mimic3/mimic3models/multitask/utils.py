...
class BatchGenTime(object):
    def __init__(self, reader, discretizer, normalizer, ihm_pos, partition,
                 target_repl, batch_size, small_part, shuffle, max_seq_len, 
                 mask_value, use_time, return_names=False):
        self.discretizer = discretizer
        self.normalizer = normalizer
        self.ihm_pos = ihm_pos
        self.partition = partition
        ...
        for i in range(N):
            if i % 1000 == 0:
                print('Processed {} / {} samples'.format(i, N), end='\r')
            self.data['decomp_ts'].append([pos for pos, m in enumerate(decomps[i][0]) if m == 1])
            self.data['los_ts'].append([pos for pos, m in enumerate(loss[i][0]) if m == 1])
            (Xs[i], Ts[i], ihms[i], decomps[i], loss[i], phenos[i]) = \
                self._preprocess_single(Xs[i], ts[i], ihms[i], decomps[i], loss[i], phenos[i], self.max_seq_len, self.mask_value)
        ...
        self.data['ihm_M'] = [x[0] for x in ihms]
        self.data['ihm_y'] = [x[1] for x in ihms]
        self.data['decomp_y'] = decomps
        self.data['los_y'] = loss
        self.data['pheno_y'] = phenos

        self.generator = self._generator()

    def _preprocess_single(self, X, max_time, ihm, decomp, los, pheno, max_seq_len, mask_value):
        eps = 1e-6

        # X
        X, T = self.discretizer.transform(X, end=max_time)[:2]
        if self.normalizer is not None:
            X = self.normalizer.transform(X)
        if len(X) < max_seq_len:
            X = np.concatenate((X, mask_value + np.zeros((max_seq_len-len(X), X.shape[1]))))
            T = np.concatenate((T, mask_value + np.zeros((max_seq_len-len(T)))))
        else:
            X = X[:max_seq_len]
            T = T[:max_seq_len]
        ...
        return (X, T, ihm, decomp, los, pheno)
