...
def preprocess_chunk(data, ts, discretizer, normalizer=None, max_seq_len=1200, mask_value=0.):
...
def preprocess_chunk_time(data, ts, discretizer, normalizer=None, max_seq_len=1200, mask_value=0.):
...
class BatchGen(object):
    ...
    def __init__(self, reader, discretizer, normalizer,
                 batch_size, steps, max_seq_len, mask_value, shuffle, return_names=False):
        self.reader = reader
    ...
    def _generator(self):
        ...
        while True:
            ...
            while remaining > 0:
                ...
                Xs = preprocess_chunk(Xs, ts, self.discretizer, self.normalizer, self.max_seq_len, self.mask_value)
...
class BatchGenTime(object):
    ...
    def __init__(self, reader, discretizer, normalizer,
                 batch_size, steps, max_seq_len, mask_value, shuffle, use_time, return_names=False):
        self.reader = reader
        self.discretizer = discretizer
        self.normalizer = normalizer
        self.batch_size = batch_size
    ...
    def _generator(self):
        ...
        while True:
            ...
            while remaining > 0:
                ...
                Xs, Ts = preprocess_chunk_time(Xs, ts, self.discretizer, self.normalizer, self.max_seq_len, self.mask_value)
                (Xs, Ts, ys, ts, names) = common_utils.sort_and_shuffle([Xs, Ts, ys, ts, names], B)
