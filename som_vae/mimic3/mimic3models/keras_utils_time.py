...
class DecompensationMetrics(keras.callbacks.Callback):
    def __init__(self, train_data_gen, val_data_gen, deep_supervision, use_time=False,
...
class InHospitalMortalityMetrics(keras.callbacks.Callback):
    def __init__(self, train_data, val_data, target_repl, batch_size=32, use_time=False, early_stopping=True, verbose=2):
    ...
    def calc_metrics(self, data, history, dataset, logs):
        ...
        predictions = []
        B = self.batch_size
        for i in range(0, len(data[0][0]), B):
            if self.verbose == 1:
                print("\tdone {}/{}".format(i, len(data[0][0])), end='\r')
            if self.target_repl:
                (x, t, y, y_repl) = (data[0][0][i:i + B], data[0][1][i:i + B], data[1][0][i:i + B], data[1][1][i:i + B])
            else:
                (x, t, y) = (data[0][0][i:i + B], data[0][1][i:i + B], data[1][i:i + B])
            
            if self.use_time:
                outputs = self.model.predict([x, t], batch_size=B)
            else:
...
class PhenotypingMetrics(keras.callbacks.Callback):
    def __init__(self, train_data_gen, val_data_gen, batch_size=32, use_time=False,
    ...
    def calc_metrics(self, data_gen, history, dataset, logs):
        ...
        for i in range(data_gen.steps):
            ...
            if self.use_time:
                ([x, t], y) = next(data_gen)
                outputs = self.model.predict([x, t], batch_size=self.batch_size)
            else:
                (x, y) = next(data_gen)
...
class LengthOfStayMetrics(keras.callbacks.Callback):
    def __init__(self, train_data_gen, val_data_gen, partition, batch_size=32, use_time=False,
                 early_stopping=True, verbose=2):
...
class MultitaskMetrics(keras.callbacks.Callback):
    def __init__(self, train_data_gen, val_data_gen, partition, use_time=False,
                 batch_size=32, early_stopping=True, verbose=2):
    ...
    def calc_metrics(self, data_gen, history, dataset, logs):
        ...
        for i in range(data_gen.steps):
            ...
            # decomp
            for (t, p) in zip(decomp_t.flatten(), decomp_p.flatten()):
                decomp_y_true.append(t)
                decomp_pred.append(p)

            # los
            for (t, p) in zip(los_t.flatten(), los_p.flatten()):
                los_y_true.append(t)
                los_pred.append(p)
